...
const port = process.env.PORT || 3000;  // Use the environment's PORT if available
...
app.post('/api/LaptopDetails', async (req, res) => {
    try {
        ...
        const pool = await poolPromise;
        const {
            model, deviceBrand, assetId, processor, laptopId, installedRam,
        ...
        if ( !model || !deviceBrand || !laptopId || !serialNumber || !assetId || !warentyMonths || !purchasedDate || !purchaseCompany || !screenSize || !resolution) {
        ...
        const laptopDetailsQuery = `
            INSERT INTO LaptopDetails1 (
                Model, DeviceBrand, AssetID, Processor, LaptopId, InstalledRAM, SerialNumber,
        ...
        const deviceDetailsQuery = `
            INSERT INTO DeviceDetails1 (
                Device, , DeviceBrand, DeviceID, Model, SerialNumber, SystemType, InvoiceNumber,
                PurchaseDate, PurchaseAmount, PurchaseCompany, WarentyMonths, WarrentyExpieryDate, CurrentStatus,
                ConditionStatus, SysDate
            ) VALUES (
                'Laptop', @AssetID, @DeviceBrand, @LaptopId, @Model, @SerialNumber, @SystemType, @InvoiceNumber,
